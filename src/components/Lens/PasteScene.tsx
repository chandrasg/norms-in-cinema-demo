...
export default function PasteScene({ endpoint }: { endpoint?: string }) {
  ...
  return (
    <div className="panel p-6 md:p-8">
      ...
      <label htmlFor="paste-scene" className="sr-only">Paste a scene of dialogue to read it through both cultural lenses</label>
      <textarea
        id="paste-scene"
        value={text}
        ...
        placeholder={SAMPLE}
        rows={8}
        className="mt-5 block w-full rounded-xl bg-ink-950 px-4 md:px-5 py-3 md:py-4 text-white placeholder-white/50 ring-1 ring-white/20 focus:outline-none focus:ring-gold-400/50 font-mono text-sm leading-relaxed"
      />
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        {text && (
          <button
            ...
            className="text-xs text-white/55 hover:text-white min-h-[40px] px-2"
          >
            Clear
          </button>
        )}
...
function AnalysisView({ result }: { result: AnalysisResult }) {
  ...
  return (
    <div className="mt-8 space-y-6">
            ...
            <div className="flex-1 rounded-xl bg-bolly/10 ring-1 ring-bolly/20 p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {result.shame_markers.map((m, i) => (
                  <span key={i} className="rounded-full bg-bolly/25 ring-1 ring-bolly/40 px-3 py-1 text-xs text-white">
                    "{m}"
                  </span>
                ))}
              </div>
            ...
            <div className="flex-1 rounded-xl bg-holly/10 ring-1 ring-holly/20 p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {result.pride_markers.map((m, i) => (
                  <span key={i} className="rounded-full bg-holly/25 ring-1 ring-holly/40 px-3 py-1 text-xs text-white">
                    "{m}"
                  </span>
                ))}
              </div>
      ...
      <p className="text-xs text-white/55 italic leading-relaxed pt-2 border-t border-white/5">
